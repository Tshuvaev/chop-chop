...
function App() {
  ...
  const handleSupportDeveloper = () => {
    if (!SUPPORT_URL) {
      setError(copy.supportLinkMissing);
      return;
    }
    ...
    try {
      const payload = {
        segments: segments.map((segment, idx) => ({
          id: idx + 1,
          name: `sample_${String(idx + 1).padStart(3, "0")}`,
    ...
    <main className="mx-auto w-full max-w-6xl overflow-x-hidden px-4 pb-8 pt-5 sm:px-5 md:px-8 md:pt-8">
      <section className="pixel-panel w-full p-3 sm:p-4 md:p-6">
        ...
        <div className="grid gap-2 md:grid-cols-[1fr_auto]">
          ...
          <button type="button" disabled={loading} onClick={handleAnalyze} className="pixel-btn pixel-btn-primary">
            {loading ? (
              <span className="inline-flex items-center gap-2">
                <span className="inline-flex items-end gap-[2px]" aria-hidden="true">
                  <span className="h-2 w-[3px] animate-pulse bg-black [animation-delay:0ms]" />
                  <span className="h-3 w-[3px] animate-pulse bg-black [animation-delay:120ms]" />
                  <span className="h-2 w-[3px] animate-pulse bg-black [animation-delay:240ms]" />
                </span>
        ...
        {loading && (
          <p className="mt-2 text-[20px] leading-none text-[var(--pixel-muted)]">
            {copy.loadingHint}
          </p>
        )}
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div className="space-y-1">
              ...
              <p className="pixel-meta text-[var(--pixel-muted)]">
                {title} // {duration.toFixed(2)}s // {segments.length} {copy.slices}
              </p>
            </div>
