...
def export_samples_to_zip(
    ...
    for idx, segment in enumerate(segments, start=1):
        ...
        _cut_wav_segment(source_wav_path, output_file, start, end, pitch=int(segment.pitch or 0))
...
def _cut_wav_segment(source: Path, destination: Path, start: float, end: float, pitch: int = 0) -> None:
    audio_filters = []
    if pitch != 0:
        factor = 2 ** (pitch / 12.0)
        shifted_rate = int(round(48000 * factor))
        tempo_correction = 1.0 / factor
        audio_filters.append(f"asetrate={shifted_rate},aresample=48000,atempo={tempo_correction:.6f}")
